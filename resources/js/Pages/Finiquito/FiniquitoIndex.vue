<template>
    <div class="max-w-lg mx-auto mt-20 p-6 bg-gray-100 rounded-lg shadow-md">
        <h1 class="text-2xl font-bold mb-6 text-center">Calculadora Finiquito</h1>
        <form @submit.prevent="buscarUsuario" class="flex mb-6 gap-2">
            <input v-model="fecha" type="date">
            <input v-model="query" type="text" placeholder="Nombre o ID del usuario" class="flex-1 p-2 border border-gray-300 rounded-l-lg focus:outline-none" />
            <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded-r-lg hover:bg-blue-600 focus:outline-none">Buscar</button>
        </form>
        ...
        <div v-if="$page.props.finiquito">
            ...
            <p class="text-red-500 font-bold">Fecha Baja: <span>{{ $page.props.finiquito.fecha_baja }}</span></p>
            <p>Vacaciones: {{ $page.props.finiquito.dias_vacaciones }}</p>
            <hr class="m-3">
            <p>Aguinaldo Proporcional: {{ $page.props.finiquito.aguinaldo_proporcional }}</p>
            ...
            <hr class="m-3">
            <p>Vacaciones: {{ $page.props.finiquito.vacaciones }}</p>
            <p>Meses Trabajados: {{ $page.props.finiquito.meses_trabajados }}</p>
            <p>Total Vacaciones Restantes: {{ $page.props.finiquito.total_vacaciones_restantes }}</p>
            <p>Prima Vacacional: {{ $page.props.finiquito.prima_vacacional }}</p>
...
export default {
    setup() {
        ...
        const buscarUsuario = () => {
            ...
            Inertia.get('/finiquito', { query: query.value, fecha: fecha.value }, {
                onSuccess: (page) => {
        ...
        const calcularFiniquito = () => {
            Inertia.post('/finiquitocontroller', { usuario_id: usuario.value.id }, {
                onSuccess: (page) => {
                    console.log('Finiquito del usuario: ', page.props.finiquito)
                    finiquito.value = page.props.finiquito;
                }
            });
        };

        return { fecha, query, usuario, finiquito, buscarUsuario, calcularFiniquito };
    },
};
</script>
