<template>
    <div class="m-2" >
        ...
        <data-table>
            <template #table-header>
                <th scope="col" class="w-3/12 text-xs font-semibold tracking-wider text-left text-gray-500 uppercase">
                    <span class="inline-flex justify-between w-full px-6 py-3"
                        @click="sort('')">Acciones
                        <svg  v-if="filters.field === 'nombre' && filters.direction === 'asc'"
                        xmlns="http://www.w3.org/2000/svg" class="w-4 h-4"
                        viewBox="0 0 20 20" fill="currentColor">
                            <path d="M3 3a1 1 0 000 2h11a1 1 0 100-2H3zM3 7a1 1 0 000 2h5a1 1 0 000-2H3zM3 11a1 1 0 100 2h4a1 1 0 100-2H3zM13 16a1 1 0 102 0v-5.586l1.293 1.293a1 1 0 001.414-1.414l-3-3a1 1 0 00-1.414 0l-3 3a1 1 0 101.414 1.414L13 10.414V16z"/>
                        </svg>
                        <svg  v-if="filters.field === 'activo' && filters.direction === 'desc'"
                ...
                <th scope="col" class="w-3/12 text-xs font-semibold tracking-wider text-left text-gray-500 uppercase">
                    <span class="inline-flex justify-between w-full px-6 py-3"
                        ...
                        <svg  v-if="filters.field === 'nombre' && filters.direction === 'desc'"
                        xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 20 20"  fill="currentColor">
                            <path d="M3 3a1 1 0 000 2h11a1 1 0 100-2H3zM3 7a1 1 0 000 2h7a1 1 0 100-2H3zM3 11a1 1 0 100 2h4a1 1 0 100-2H3zM15 8a1 1 0 10-2 0v5.586l-1.293-1.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L15 13.586V8z"/>
                        </svg>
                    </span>
                ...
                <th scope="col" class="w-3/12 text-xs font-semibold tracking-wider text-left text-gray-500 uppercase">
                    <span class="inline-flex justify-between w-full px-6 py-3"
                        @click="sort('activo')">Activo
                        <svg  v-if="filters.field === 'activo' && filters.direction === 'asc'"
                        xmlns="http://www.w3.org/2000/svg" class="w-4 h-4"
                        viewBox="0 0 20 20" fill="currentColor">
                            <path d="M3 3a1 1 0 000 2h11a1 1 0 100-2H3zM3 7a1 1 0 000 2h5a1 1 0 000-2H3zM3 11a1 1 0 100 2h4a1 1 0 100-2H3zM13 16a1 1 0 102 0v-5.586l1.293 1.293a1 1 0 001.414-1.414l-3-3a1 1 0 00-1.414 0l-3 3a1 1 0 101.414 1.414L13 10.414V16z"/>
                        </svg>
            ...
            <template #table-body>
                <tr v-for='(puesto, index) in puestos.data' :key="index">
                    <td  class="flex w-8 gap-1 px-2 py-4 text-sm">
                        <input type="checkbox"
                        ...
                        <div v-for="pustoDep in puestosDepartamento" :key="pustoDep.id">
                            <input v-if="pustoDep.puesto_id == puesto.id" type="checkbox" disabled checked/>
                        </div>
                    
                    </td>
                    <td  class="w-4/12 px-2 py-4 text-sm text-gray-500 uppercase break-all">
                        {{ puesto.name }}
                    </td>
                    <td  class="w-7/12 px-2 py-4 text-sm text-center text-gray-500 uppercase break-all">
                        <svg v-if="puesto.activo" class="w-6 h-6 m-auto text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <svg v-else class="w-6 h-6 m-auto text-red-500" xmlns="http://www.w3.org/2000/svg"  fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <puesto-modal :show="showingPuesto" :puesto="puestoSelect"
        :typeForm="typeForm" @close="closeModalPuesto" @savedPuesto="searchPuestos"/>
        <jet-dialog-modal :show="showingConfirmDeletion" @close="closeModalDelete">
                <template #title>
                    Eliminar Puesto
                </template>

                <template #content>
                    Estas seguro que deseas eliminar el puesto?, Una vez que se elimine, este ya
                ...
                <template #footer>
                    ...
                    &nbsp;
                    <danger-button @click="deletePuesto">Eliminar
                    </danger-button>
                </template>
        </jet-dialog-modal>
    </div>
...
export default {
  ...
  methods: {
        async setPuestoToDepartamento(puestoId, checked){
             await axios.put(route('departamento.puestos.update', this.departamento.id),{
                    puesto_id: puestoId,
                    checked
             }).catch( error =>{
                    if (error.response) {
                        ...
                        swal('Error SET Puesto TO Departamento', messageError, 'error');
                    }
                })
        },
        async getPuestosDepartemento(){
            await axios.get(route('departamento.puestos.index', this.departamento.id))
                .then( (response) => {
        ...
        async getPuestos(){
            await axios.get(this.route('puestos.index'), {
        ...
        deletePuesto() {
            axios.delete(this.route('puestos.destroy', this.puestoSelect.id)).
            then( () => {
                this.searchPuestos();
