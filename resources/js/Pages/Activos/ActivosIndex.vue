...
<template>
    <AppLayout title="Activos" :nominas="nominas">
        <div
            class="grid w-full grid-cols-5 grid-rows-3 gap-4 activo_banner h-96"
        >
            <div class="col-end-3 mt-4">
                <h1
                    class="mt-2 text-3xl text-white"
            ...
            <div class="col-start-2 col-end-5 pt-8">
                <p class="text-xl text-white" style="font-family: 'Montserrat'">
                    En esta sección tendrás acceso a la información de los
                    activos asignados.
                </p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-5 mt-8 mb-8">
            ...
            <ButtonAdd
                class="flex justify-center col-start-4 rounded-none"
                @click="openModalNewCategory"
            >
                <span class="text-base" style="letter-spacing: 0.25rem"
                    >Agregar categoría</span
                >
            </ButtonAdd>
            <ModalAddCategoria
                :tipo_inputs="tipo_inputs"
                :show="modalNewCategory"
                @close="closeModalNewCategory"
            ></ModalAddCategoria>
        </div>
        <div class="grid grid-cols-7">
            <ActivoConteo
                v-for="tipoActivo in props.tipo_activos"
                :key="tipoActivo.id"
