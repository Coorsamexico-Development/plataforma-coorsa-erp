...
export function ObtenerCurp(
    ...
    const PRIMERALETRAPRIMERAPELLIDO =
        LETTERS_APELLIDO_PATERNO.length > 0 ? LETTERS_APELLIDO_PATERNO[0] : "";

    //primera vocal del apellido paterno considerando la primer letra
    let VOCALPRIMERAPELLIDO = LETTERS_APELLIDO_PATERNO.find(
        (letter, index) => vocales.includes(letter) && index > 0
    );
    VOCALPRIMERAPELLIDO =
        VOCALPRIMERAPELLIDO === undefined ? "" : VOCALPRIMERAPELLIDO;

    //primera letra del segundo apellido
    const LETTERS_APELLIDO_MATERNO = _apellidoMaterno.split("");
    const PRIMERALETRASEGUNDOAPELLIDO =
        LETTERS_APELLIDO_MATERNO.length > 0 ? LETTERS_APELLIDO_MATERNO[0] : "";
    ...
    return (
        PRIMERALETRAPRIMERAPELLIDO +
        VOCALPRIMERAPELLIDO +
        PRIMERALETRASEGUNDOAPELLIDO +
        PRIMERALETRAPRIMERNOMBRE +
        FECHANACIMIENTO
    );
}
...
export async function HandlerQueryBuscador(
    _cadenaBusqueda,
    _ballesta,
    _tipoBusqueda
) {
    ...
    if (_tipoBusqueda !== "")
        response = await axios.get(
            _cadenaBusqueda + `/${_ballesta}?orden=${_tipoBusqueda}`
        );
